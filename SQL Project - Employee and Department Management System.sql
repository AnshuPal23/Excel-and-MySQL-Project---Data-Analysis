...
INSERT INTO Departments (DepartmentName, Location, HeadOfDepartment, AnnualBudget)
VALUES (' Research department', 'Hyderabad', 'Dr. Meera Joshi', 2000000);
...
select * from Employees
where DateOfJoining < '2024-03-01'and
 Salary > 55000;
...
-- department. 
select DepartmentID, min(DateOfJoining) as joiningdate
from Employees
group by DepartmentID;
...
-- 2024 in each department. 
 select * from Employees;
Select DepartmentID, count(DateOfJoining) as 2024department
from Employees
...
Select DepartmentID, count(Salary) as MidLevelEmployees
from Employees
where Salary between 50000 and 70000
group by DepartmentID;
...
-- Assuming PhoneNumber and Extension columns exist
SELECT concat (PhoneNumber,IFNULL(CONCAT('EXT-', Extension),'')) AS ContactDetails
FROM Employees;
...
SELECT CONCAT(FirstName, LastName, ', ',DepartmentID) AS employeeloginID
FROM Employees;
...
select * from employees;
SELECT CONCAT(FirstName,'', ' ', LastName,'') AS WorkStatus
FROM Employees;
...
update employees
set projectassignment = 'Project Alpha'
...
delete from employees where DepartmentID= 2 and DateOfJoining < '2022';
...
active projects should be removed from the system. Delete these employee records.*/
 ...
 delete from employees where DateOfJoining < 2020  AND ProjectName IS NULL;
...
delete from employees where Email is null and contactnumbers is null;
...
delete from employees where Salary > 100000 AND ContractStatus = 'Expired';
...
DELETE FROM Employees WHERE Region = 'North' AND EndDate < '2023-01-01';
...
remains accurate for all future entries.*/ 
 ...
 alter table Employees add marital_status VARCHAR(20);
 select * from Employees;
...
Note: Please ensure the changes are reflected and retrieve the updated schema for consistency. */
 ...
 alter table Employees add EmployeeCategory ENUM ('Full-time', 'Part-time', 'Contract');
 select * from Employees;
 
INSERT INTO Employees 
(FirstName, Salary, Email, EmployeeCategory)
VALUES 
('Amit', 50000, 'amit.new@example.com', 'Full-time');
